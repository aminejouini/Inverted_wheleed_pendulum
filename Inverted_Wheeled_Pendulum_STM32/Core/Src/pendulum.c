...
extern float E_kin,E_pot,E_tar;
...
void pendulum_LQR(){
	...
	* Q = diag([1,1,1]) R = [10]
	*/
	float Kr[3] = {600.1146,35.1388,0.3}; //330.1146 35.1388
	//float Kr[3] = {123.3941,13.1524,0.1};


	// P equation
	//float uk = 60000*pendulum.position;
	// LQR equation
	float uk = Kr[0]*pendulum.position+Kr[1]*pendulum.velocity +Kr[2]*uk ;//+Kr[2]*uk;
...
//		}
		if(fabs(pendulum.velocity) < 5.0){
					// if total energy too small accelerate
					accel_sign = 1;
				}else if(fabs(pendulum.velocity) < 10.0){
					// if in the area of target energy do nothing
					accel_sign = 0;
				}else{
					// if total energy too high decelerate
